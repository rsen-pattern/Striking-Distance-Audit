...
.badge-monitor {
    background: #3a3a1a; color: #cfcf6f;
    padding: 2px 10px; border-radius: 12px;
    font-size: 0.75rem; font-weight: 600;
    display: inline-block; margin: 2px;
}
...
def tag_block(label: str, value: str, recommended: bool = False, show_chars: bool = True) -> str:
    cls = "recommended" if recommended else ""
    char_info = f'<div class="char-count">{len(value)} chars</div>' if show_chars and value else ""
    safe_value = value.replace("<", "&lt;").replace(">", "&gt;") if value else "<em style='color:#555'>—</em>"
    return f"""
    <div class="tag-block">
        <div class="tag-label">{label}</div>
        <div class="tag-value {cls}">{safe_value}</div>
        {char_info}
    </div>"""


def decision_badge(decision: str) -> str:
    cls = {"OPTIMISE": "badge-optimise", "REPLACE": "badge-replace", "MONITOR": "badge-monitor"}.get(decision, "badge-monitor")
...
def render_sidebar() -> dict:
    ...
    semrush_key = st.sidebar.text_input(
        "Semrush API Key",
        value=st.secrets.get("SEMRUSH_KEY", ""),
        type="password",
        help="From Semrush → API → API Keys",
    )
    bifrost_key = st.sidebar.text_input(
        "Bifrost / OpenAI Key",
        value=st.secrets.get("BIFROST_KEY", ""),
        type="password",
        help="Your Bifrost sk-bf-... key or OpenAI sk-... key",
    )
    bifrost_base_url = st.sidebar.text_input(
        "AI Base URL",
        value="https://bifrost.pattern.com/v1",
        help="Change to https://api.openai.com/v1 for direct OpenAI",
    )
    ...
    upload_mode = st.sidebar.radio(
        "Input method",
        ["Upload Excel file", "Google Sheets CSV URLs"],
        index=0,
    )
    ...
    if upload_mode == "Upload Excel file":
        excel_file = st.sidebar.file_uploader(
            "Upload Excel (.xlsx)",
            type=["xlsx"],
            help="Sheet tabs: SEMRUSH_Ranking KWs · Internal_All · BrandRules",
        )
        st.sidebar.caption("Required sheet tabs: **SEMRUSH_Ranking KWs**, **Internal_All**, **BrandRules**")
    else:
        semrush_csv_url = st.sidebar.text_input("Semrush Rankings CSV URL", placeholder="https://docs.google.com/...")
        crawl_csv_url = st.sidebar.text_input("Crawl Data CSV URL", placeholder="https://docs.google.com/...")
        brand_csv_url = st.sidebar.text_input("BrandRules CSV URL", placeholder="https://docs.google.com/...")
    ...
    semrush_db = st.sidebar.selectbox("Semrush database", ["us", "au", "uk", "ca", "nz"], index=0)
    max_urls = st.sidebar.number_input("Max URLs to process (0 = all)", min_value=0, value=0, step=10)

    st.sidebar.markdown("---")
    st.sidebar.markdown("### Google Sheets Export *(optional)*")
    output_sheet_url = st.sidebar.text_input("Output Sheet URL", placeholder="https://docs.google.com/...")
    ...
    return {
        ...
        "semrush_csv_url": semrush_csv_url,
        "crawl_csv_url": crawl_csv_url,
        "brand_csv_url": brand_csv_url,
        "min_pos": int(min_pos),
        "max_pos": int(max_pos),
        "min_sv": int(min_sv),
...
def run_audit(cfg: dict, status_placeholder, progress_bar, log_placeholder) -> list[dict]:
    """Execute the full audit pipeline. Returns processed_results list."""
    logs = []
    ...
    # Stage 1 — Load data
    status_placeholder.markdown("### Stage 1 — Loading data...")
    ...
    try:
        ...
        log(f"✓ Semrush: {len(semrush_df)} keywords loaded")
    ...
    try:
        ...
        log(f"✓ Crawl: {len(crawl_df)} indexable pages loaded")
    except Exception as exc:
    ...
    try:
        ...
        log(f"✓ Brand rules loaded for: {brand_rules['brand_name']}")
    except Exception as exc:
    ...
    status_placeholder.markdown("### Stage 2 — Filtering & scoring keywords...")
    ...
    log(f"✓ {len(url_groups)} URLs in striking distance")
    progress_bar.progress(25)

    # Determine own domain from first URL
    ...
    for i, ug in enumerate(url_groups):
        ...
        # Stage 3 — SERP fetch
        status_placeholder.markdown(f"### Stage 3 — SERP fetch ({i+1}/{total})")
        log(f"[{i+1}/{total}] SERP: '{primary_kw}' → {url}")
        ...
        if cfg["semrush_key"]:
            try:
                ...
                log(f"  ✓ {len(raw_competitors)} competitors found")
            except Exception as exc:
                log(f"  ⚠ SERP fetch failed: {exc}")
        else:
            log("  ⚠ No Semrush key — skipping SERP fetch")

        # Stage 4 — Scrape competitors
        status_placeholder.markdown(f"### Stage 4 — Scraping competitors ({i+1}/{total})")
        competitors = []
        if raw_competitors:
            try:
                competitors = scrape_competitors(raw_competitors)
                ok = sum(1 for c in competitors if not c.get("error"))
                log(f"  ✓ Scraped {ok}/{len(competitors)} competitor pages")
            except Exception as exc:
                log(f"  ⚠ Competitor scraping failed: {exc}")

        # Stage 5 — AI recommendations
        status_placeholder.markdown(f"### Stage 5 — Generating AI recommendations ({i+1}/{total})")
        ai_result = None
        if cfg["bifrost_key"]:
            try:
                ...
                if ai_result:
                    log(f"  ✓ AI recommendations generated")
                else:
                    log(f"  ⚠ AI returned no result")
            except Exception as exc:
                log(f"  ⚠ AI generation failed: {exc}")
        else:
            log("  ⚠ No AI key — skipping recommendations")
    ...
    status_placeholder.markdown("### Stage 6 — Building output...")
...
def render_url_card(item: dict):
    ...
    with st.expander(label, expanded=False):
        badges = (
            ...
            f'{total_kws} KWs in range · primary: <em>{ug.get("primary_keyword","")}</em>'
...
def main():
    ...
    if cfg["run_audit"]:
        ...
        with st.spinner("Running audit..."):
            processed = run_audit(cfg, status_ph, progress_ph, log_ph)
        ...
        if processed:
            ...
            if cfg["output_sheet_url"] and cfg["service_account_json"]:
                try:
                    import json
                    creds = json.loads(cfg["service_account_json"])
                    ok = export_to_gsheet(output_df, cfg["output_sheet_url"], creds)
                    if ok:
    ...
    if st.session_state.output_df is not None and not st.session_state.output_df.empty:
        ...
        with dl_col1:
            excel_bytes = to_excel_bytes(st.session_state.output_df)
            st.download_button(
                "⬇ Download Excel",
                data=excel_bytes,
                file_name=f"striking_distance_audit_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                use_container_width=True,
            )
        with dl_col2:
            csv_bytes = st.session_state.output_df.to_csv(index=False).encode()
            st.download_button(
                "⬇ Download CSV",
                data=csv_bytes,
                file_name=f"striking_distance_audit_{datetime.now().strftime('%Y%m%d_%H%M')}.csv",
                mime="text/csv",
                use_container_width=True,
            )
    ...
    if st.session_state.processed_results:
        st.markdown(f"### Results — {len(st.session_state.processed_results)} URLs")
